...
export class CreateEditCategoryComponent implements OnInit,OnDestroy{
  ...
  constructor(
    ...
    categoryType?:string
    catId?:number
    parents$?:Observable<IMainCategory[]|IOneLevelSubCategory[]>
    errorText?:string

    ngOnInit(): void {
      this.activatedRoute.params.subscribe(d=>{
        this.categoryType=d['categoryType'];
        this.catId=parseInt(d['id'])   
        switch(this.categoryType){        
          case 'main_categories': 
           // if componet works in edit mode set form
           if(this.catId){
             let editCategory=this.mainCategoryService.entries$.getValue().find((cat:IMainCategory)=>cat.id===this.catId)
             if(editCategory){
          ...
          case 'one_level_sub_categories':         
            this.parents$=this.mainCategoryService.entries$
            // if "this.catId" exists component works in edit mode 
            if(this.catId){
              let editCategory=this.oneLevelSubCategoryService.entries$.getValue().find((cat:IOneLevelSubCategory)=>cat.id===this.catId)
              if(editCategory){
          ...
          case 'two_level_sub_categories':
            this.parents$=this.oneLevelSubCategoryService.entries$
            if(this.catId){
              let editCategory=this.twoLevelSubCategoryService.entries$.getValue().find((cat:ITwoLevelSubCategory)=>cat.id===this.catId)
              if(editCategory){
                this.form.patchValue({
                  title:editCategory.attributes.title,
                  parent:editCategory.attributes.one_level_sub_categories?.data[0].id
                })             
              }else{
                console.error("don't found requested entry in state")
              }
  ...
  submit() {    
    ...
    switch(this.categoryType){ 
      ...
      case 'main_categories':
          ...
          if(this.catId){
            this.mainCategoryService.update(this.catId,body).pipe(takeUntil(this.unsubscribe$))
              .subscribe(
                {
                  next:(cat:IMainCategory)=>{
                    let categories:IMainCategory[]=this.mainCategoryService.entries$.getValue()
                    let index=categories.findIndex((cat:IMainCategory)=>cat.id===this.catId)
                    if(index>=0){
      ...
      case 'one_level_sub_categories':      
       ...
       if(this.catId){
        this.oneLevelSubCategoryService.update(this.catId,body)
        .pipe(
          ...
          .subscribe(
            {
              next:(cat:IOneLevelSubCategory)=>{
                let categories:IOneLevelSubCategory[]=this.oneLevelSubCategoryService.entries$.getValue()
                let index=categories.findIndex((cat:IOneLevelSubCategory)=>cat.id===this.catId)
                if(index>=0){
                  categories[index]={...categories[index],...cat}
      ...
      case 'two_level_sub_categories':
        ...
        if(this.catId){
          this.twoLevelSubCategoryService.update(this.catId,body)
        .pipe(
          takeUntil(this.unsubscribe$),
          ...
          .subscribe(
            {
              next:(cat:ITwoLevelSubCategory)=>{
                let categories:ITwoLevelSubCategory[]=this.twoLevelSubCategoryService.entries$.getValue()
                let index=categories.findIndex((cat:ITwoLevelSubCategory)=>cat.id===this.catId)
                if(index>=0){
                  categories[index]={...categories[index],...cat}
  ...
  GoToBack():void{
    ...
    if(this.catId){
      url=this.router.url.split('/').slice(0,length-2).join('/')
    }
    // component works in create mode
    else{
      url=this.router.url.split('/').slice(0,length-1).join('/')
    }
